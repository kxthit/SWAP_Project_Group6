...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (!$stmt->execute()) {
            // If execute fails, display error and stop the script
            echo "Error updating student: " . $stmt->error;
            exit; // Stop further execution
        }
        $stmt->close();

        // Step 2: Update course mappings (if needed)
        foreach ($selected_courses as $course_id) {
            $stmt = $conn->prepare("INSERT INTO student_course (student_id, course_id) VALUES (?, ?) ON DUPLICATE KEY UPDATE course_id = ?");
            $stmt->bind_param("iii", $_SESSION['session_userid'], $course_id, $course_id);
            if (!$stmt->execute()) {
                // If execute fails, display error and stop the script
                echo "Error updating course mapping: " . $stmt->error;
                exit; // Stop further execution
            }
            $stmt->close();
        }

        // Step 3: Update class mappings (if needed)
        foreach ($selected_classes as $course_id => $class_ids) {
            foreach ($class_ids as $class_id) {
                $stmt = $conn->prepare("INSERT INTO student_class (student_id, class_id) VALUES (?, ?) ON DUPLICATE KEY UPDATE class_id = ?");
                $stmt->bind_param("iii", $_SESSION['session_userid'], $class_id, $class_id);
                if (!$stmt->execute()) {
                    // If execute fails, display error and stop the script
                    echo "Error updating class mapping: " . $stmt->error;
                    exit; // Stop further execution
                }
                $stmt->close();
            }
