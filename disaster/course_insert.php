...
// Query the database to get the faculty_id associated with the logged-in user
...
if ($get_faculty_id_result->num_rows > 0) {
    ...
    // If no faculty_id found, redirect or show an error message
...
if (isset($_POST['course_name'], $_POST['course_code'], $_POST['start_date'], $_POST['end_date'], $_POST['status_id'], $_POST['department_id'])) {
...
// If there are any validation errors, store them in the session and redirect back to the form
if (!empty($errors)) {
    $_SESSION['error_message'] = $errors;  // Store error messages
    $_SESSION['course_data'] = $_POST;  // Store the submitted course data in session for form repopulation
...
// If a course with the same name exists, return an error
if ($check_name_result->num_rows > 0) {
    $errors[] = "A course with the same name already exists.";  // Add the error to the array
    $_SESSION['error_message'] = $errors;  // Store the errors in session
    $_SESSION['course_data'] = $_POST;  // Store the submitted course data in session
    header('Location: course_insertform.php');  // Redirect back to the form
    exit;
}
...
// If a course with the same code exists, return an error
if ($check_code_result->num_rows > 0) {
    $errors[] = "A course with the same course code already exists.";  // Add the error to the array
    $_SESSION['error_message'] = $errors;  // Store the errors in session
    $_SESSION['course_data'] = $_POST;  // Store the submitted course data in session
    header('Location: course_insertform.php');  // Redirect back to the form
    exit;
...
$course_id = $insert_stmt->insert_id; // The ID of the newly inserted course

// Check if the insertion was successful
if ($insert_stmt->affected_rows > 0) {
    // Now insert into the faculty_course table to establish the relationship
    ...
    // Check if the faculty_course insertion was successful
    if ($faculty_course_stmt->affected_rows > 0) {
        // Clear session data after successful insert
        unset($_SESSION['course_data']);  // Clear course data
