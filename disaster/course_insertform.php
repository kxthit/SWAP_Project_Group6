...
<head>
    ...
    <script>
        // Automatically unset session data when the page is about to be unloaded
        window.addEventListener("beforeunload", function() {
            <?php 
            // PHP: unset the session data before leaving
    ...
    <div class="form-container">
        ...
        <form action="course_insert.php" method="POST" onsubmit="return handleFormSubmit(event);">
            ...
            <input type="text" id="course_name" name="course_name" class="form-input"
                   value="<?php echo isset($_SESSION['course_data']['course_name']) ? htmlspecialchars($_SESSION['course_data']['course_name'], ENT_QUOTES, 'UTF-8') : ''; ?>" required>

            <label for="course_code">Course Code:</label>
            <input type="text" id="course_code" name="course_code" class="form-input"
                   value="<?php echo isset($_SESSION['course_data']['course_code']) ? htmlspecialchars($_SESSION['course_data']['course_code'], ENT_QUOTES, 'UTF-8') : ''; ?>" required>

            <label for="start_date">Start Date:</label>
            <input type="date" id="start_date" name="start_date" class="form-input"
                   value="<?php echo isset($_SESSION['course_data']['start_date']) ? $_SESSION['course_data']['start_date'] : ''; ?>" required>

            <label for="end_date">End Date:</label>
            <input type="date" id="end_date" name="end_date" class="form-input"
                   value="<?php echo isset($_SESSION['course_data']['end_date']) ? $_SESSION['course_data']['end_date'] : ''; ?>" required>
            ...
            <select id="status_id" name="status_id" class="form-select" required>
                ...
                while ($status = mysqli_fetch_assoc($status_result)) {
                    $selected = (isset($_SESSION['course_data']['status_id']) && $_SESSION['course_data']['status_id'] == $status['status_id']) ? 'selected' : '';
            ...
            <select id="department_id" name="department_id" class="form-select" required>
                ...
                while ($department = mysqli_fetch_assoc($department_result)) {
                    $selected = (isset($_SESSION['course_data']['department_id']) && $_SESSION['course_data']['department_id'] == $department['department_id']) ? 'selected' : '';
