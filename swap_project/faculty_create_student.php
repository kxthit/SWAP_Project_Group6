...
$stmt->execute();
$stmt->bind_result($faculty_department_id);
$stmt->fetch();
$stmt->close();

// Fetch department name for the faculty's department
$stmt = $conn->prepare("SELECT department_name FROM department WHERE department_id = ?");
$stmt->bind_param("i", $faculty_department_id);
$stmt->execute();
$stmt->bind_result($faculty_department_name);
$stmt->fetch();
$stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['department_id']) && $_POST['department_id'] !== '') {
        ...
        header("Location: faculty_create_student_course.php");
        exit;
    } else {
    ...
    <main class="main-content">
        ...
        <div class="form-container">
            <div class="form-card">
                ...
                <form action="faculty_create_student.php" method="POST">
                            ...
                            <tr>
                                <td colspan="2">
                                    ...
                                    <select id="department_id" name="department_id" required>
                                        <option value="<?php echo $faculty_department_id; ?>">
                                            <?php echo htmlspecialchars($faculty_department_name); ?>
                                        </option>
                                    </select>
                                </td>
                            </tr>
